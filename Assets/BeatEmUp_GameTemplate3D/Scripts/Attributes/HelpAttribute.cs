...
public class HelpDrawer : PropertyDrawer {
	private const int paddingHeight = 8; // Used for top and bottom padding between the text and the HelpBox border.
	private const int marginHeight = 2; // Used to add some margin between the the HelpBox and the property.
	private float baseHeight = 0; //  Global field to store the original (base) property height.
	private float addedHeight = 0; // Custom added height for drawing text area which has the MultilineAttribute.
	private HelpAttribute helpAttribute { get { return (HelpAttribute)attribute; } } // A wrapper which returns the PropertyDrawer.attribute field as a HelpAttribute.
