...
public class EnemyActions : MonoBehaviour
{
    ...
    public Vector3 moneyVFXSpawnOffset = new Vector3(0,1,0);
    public AnimationCurve moneyPerLevel = AnimationCurve.Linear(1,1, 100, 100);
    ...
    public bool
        canDefendDuringAttack; //true if the enemy is able to defend an incoming attack while he is doing his own attack
    ...
    public float
        RangeMarging =
            1f; //the amount of space that is allowed between the player and enemy before we reposition ourselves
    public float walkSpeed = 1.95f; //the speed of a walk
    public float walkSpeedModifier = 1f; //modifies walkSpeed & walk animation speed at OnStart
    ...
    public float attackSpeedModifier = 1f; //modifies attack animation speed at OnStart
    ...
    private List<UNITSTATE>
        defendableStates = new List<UNITSTATE>
        {
            UNITSTATE.IDLE, UNITSTATE.WALK, UNITSTATE.DEFEND
    ...
    // ntk fields
    Coroutine c_knockDownSequence;
    //Coroutine c_groundHit;
    ...
    public void OnStart()
    {
        ...
        //applying walkSpeedModifier
        walkSpeed *= walkSpeedModifier;
        animator.animator.SetFloat("Walk Speed Modifier", walkSpeedModifier);

        //applying attackSpeedModifier
        animator.animator.SetFloat("Attack Speed Modifier", attackSpeedModifier);
    }
    ...
    public void Hit(DamageObject d)
    {
        if (HitableStates.Contains(enemyState))
        {
            ...
            //hit particle effect
            ShowHitEffectAtPosition(new Vector3(transform.position.x, d.inflictor.transform.position.y + d.collHeight,
                transform.position.z), d.isCritical);
            ...
            if (enemyState == UNITSTATE.KNOCKDOWNGROUNDED ||
                enemyState == UNITSTATE.GROUNDHIT) // ntk
            {
                StopAllCoroutines();
            ...
            if (d.knockDown)
            {
                if (c_knockDownSequence != null)
                    StopCoroutine(c_knockDownSequence);

                c_knockDownSequence = StartCoroutine(KnockDownSequence(d.inflictor));
                return;
            }
            else
            {
    ...
    #endregion Check for hit

    #region KnockDown Sequence

    //knockDown sequence
    private IEnumerator KnockDownSequence(GameObject inflictor)
    {
    ...
    public void WalkTo(float proximityRange, float movementMargin)
    {
        ...
        if (enemyTactic == ENEMYTACTIC.ENGAGE)
        {
            dirToTarget = target.transform.position -
                          (transform.position + new Vector3(0, 0, Mathf.Clamp(ZSpread, 0, attackRangeDistance)));
        }
        else
        {
    ...
    public void Move(Vector3 vector, float speed)
    {
        ...
        {
            SetVelocity(new Vector3(vector.x * speed * transform.localScale.x,
                rb.velocity.y + Physics.gravity.y * Time.fixedDeltaTime, vector.z * speed * transform.localScale.z));
        }
        else
        {
    ...
    private bool WallSpotted()
    {
        ...
        Collider[] hitColliders =
            Physics.OverlapSphere(transform.position + capsule.center + Offset, capsule.radius, CollisionLayer);
    ...
    private bool PitfallSpotted()
    {
        if (!ignoreCliffs)
        {
            ...
            Vector3 StartPoint = transform.position + (Vector3.up * .3f) +
                                 (Vector3.right * (capsule.radius + lookaheadDistance) * moveDirection.normalized.x);
            RaycastHit hit;
    ...
    public bool IsGrounded()
    {
        ...
        if (Physics.CheckCapsule(capsule.bounds.center, capsule.bounds.center + Vector3.down * colliderSize,
            capsule.radius, CollisionLayer))
        {
            isGrounded = true;
            return true;
        }
    ...
    //show hit effect
    public void ShowHitEffectAtPosition(Vector3 pos, bool isCritical)
    {
        GameObject.Instantiate(isCritical ? Resources.Load("HitEffectCritical") : Resources.Load("HitEffect"), pos,
            Quaternion.identity);
    }
    
    // money vfx
    protected void MoneyOnDeath()
    {
        if (isDead)
        {
            Debug.LogWarning("Unit is already dead but you're invoking MoneyOnDeath");
            return;
        }

        var addMoney = (int) moneyPerLevel.Evaluate(GetComponent<EnemyLevelScaler>().savedLevel);
        
        // Visual
        if (moneyVFX!=null)
        {
            var VFXObject = LeanPool.Spawn(moneyVFX, transform.position + moneyVFXSpawnOffset, Quaternion.identity, transform);
            var VFX = VFXObject.GetComponent<ParticleSystem>();
            VFX.Stop();
            VFX.emission.SetBurst(0, new ParticleSystem.Burst(0, addMoney));
            VFX.Play();
        }
        
        // Audio
        GlobalAudioPlayer.PlaySFX("MoneyFromEnemy");
        
        MoneyBag.AddMoney(addMoney);
    }
    ...
    public void LookAtTarget(Transform _target)
    {
        if (_target != null)
        {
            ...
            newDir = Vector3.RotateTowards(transform.forward, Vector3.forward * dir, rotationSpeed * Time.deltaTime,
                0.0f);
    ...
    private string GetRandomName()
    {
        ...
        {
            Debug.Log(
                "no list of names was found, please create 'EnemyNames.txt' that contains a list of enemy names and put it in the 'Resources' folder.");
            return "";
        }
